            ...
            <div ref={sectionRef} className="flex h-screen w-fit items-center px-6 md:px-20 gap-8 md:gap-16 box-border">
                ...
                <div className="w-[90vw] md:w-[35vw] flex-shrink-0 px-4 md:px-0">
                    ...
                    <h2 className="text-3xl md:text-5xl font-black text-foreground leading-tight mb-4">
                        ...
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-foreground to-gray-500 dark:from-white dark:to-gray-500">
                             <AnimatedText text="The Campus" type="words" animation="reveal" delay={0.2} />
                        </span>
                    </h2>
                    <p className="text-sm md:text-lg text-gray-600 dark:text-gray-400 max-w-sm leading-relaxed">
                        A visual journey through our world-class facilities and programs.
                    </p>
                    <div className="mt-6 flex items-center gap-4 text-[10px] font-bold text-gray-600 dark:text-gray-400 animate-pulse">
                        SCROLL TO EXPLORE <div className="w-12 h-[1px] bg-gray-600 dark:bg-gray-400"></div>
                    </div>
                </div>
                ...
                {items.map((item, index) => (
                    <div 
                        key={item.id} 
                        className="w-[85vw] md:w-[500px] h-[450px] flex-shrink-0 rounded-[24px] relative overflow-hidden group border border-gray-200 dark:border-white/10 bg-white dark:bg-[#0f0f13] hover:border-primary/30 dark:hover:border-primary/30 transition-all duration-500 box-border shadow-2xl"
                    >
                        ...
                        <div className="absolute inset-0 p-8 flex flex-col justify-between z-10">
                            <div>
                                <div className="flex justify-between items-start mb-6">
                                    <div className="bg-gray-100 dark:bg-white/5 w-fit p-3 rounded-xl backdrop-blur-md border border-gray-200 dark:border-white/5">
                                        {React.cloneElement(item.icon, { className: 'w-6 h-6' })}
                                    </div>
                                    <span className="font-mono text-[10px] text-gray-500 dark:text-gray-600">0{index + 1}</span>
                                </div>
                                <AnimatedText 
                                    text={item.title} 
                                    className="text-2xl md:text-3xl font-black text-foreground dark:text-white mb-2 leading-tight block tracking-tight"
                                    type="words" 
                                    animation="fade-up" 
                                />
                            ...
                            <div className="space-y-6">
                                ...
                                <button className="px-6 py-3 bg-transparent border border-gray-300 dark:border-white/20 text-foreground dark:text-white rounded-xl font-bold uppercase tracking-widest text-[10px] hover:bg-foreground hover:text-background dark:hover:bg-white dark:hover:text-black hover:border-transparent transition-all">
                                    View Details
                                </button>
                            </div>
                        </div>
                         ...
                         <div className="absolute -bottom-6 -right-6 opacity-[0.05] transform rotate-12 scale-[2.5] pointer-events-none group-hover:opacity-[0.1] transition-opacity duration-500 text-foreground dark:text-white">
                            {item.icon}
                         </div>
                    </div>
                ))}
